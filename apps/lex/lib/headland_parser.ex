defmodule Lex.HeadlandParser do
  @docmodule "APARTADO = TITLE ( SUBAPARTADO+ | CAPITULO+ | ARTICULO+ )"

  @headlands_exist_expression ~r{I.-}
  @headland_expression ~r{\n\n\s\s\s\s\s\s\s+I+V*\.-\s.*}

  import Lex.ChapterParser,  only: [parse_chapter: 1, chapter_expression: 0]
  import Lex.ArticleParser,  only: [parse_article: 1, article_expression: 0]
  import Lex.SubHeadLandParser, only: [parse_subheadland: 1, subheadland_expression: 0]
