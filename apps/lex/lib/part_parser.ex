defmodule Lex.PartParser do
  ...
  import Lex.TitleParser, only: [parse_title: 1, title_first_expression: 0, title_expression: 0]
  ...
  def parse_part({part, index}) do
    ...
    {part_title, titles_with_index} =
    if part_has_title(part) do
      extract_part_title(titles)
    else
      {"SIN TITULO", Enum.with_index(titles)}
    end
